...
```{r}
contTable <- xtabs(~ position + position, data = pushups)
barchart(contTable,
	auto.key = list(
		space = "top",
		title = "position",
...
From studying the graph one can clearly see that the skill players are capable of more pushups versus the line players.
## Numerical Results
<!--Numerical results go here. Use the numerical results to describe the patterns if any that exist in the data as focused toward the research question!-->

pushups <- xtabs(~pushups+position,data(pushups))
pushups
```{r}
pushups <- xtabs(~pushups+position,data=pushups)
pushups
...
mytable <- xtabs(~pushups+position,data(pushups))
